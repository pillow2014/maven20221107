 ...
 */
@WebFilter(filterName = "FilterA", urlPatterns = {"/FAServlet"}) //篩選登入當網址為"/FAServlet"時才會進入
public class FilterA extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		chain.doFilter(request, response); //當request時連鎖執行
		
		long end=System.currentTimeMillis();
		System.out.println("執行時間:"+(end-start)+"ms"); //計算執行request時的連線時間然後印出
